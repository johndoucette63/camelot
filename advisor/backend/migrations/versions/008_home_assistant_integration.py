...
def upgrade() -> None:
    ...
    # Drop the existing table-level unique on mac_address. SQLAlchemy named
    # this "uq_devices_mac_address" when migration 001 was authored (its
    # naming convention is "uq_<table>_<column>"). The Postgres-autogen name
    # "devices_mac_address_key" would only apply if the column was declared
    # with `unique=True` at column level; in this project it's a table-level
    # UniqueConstraint so the uq_* name is authoritative.
