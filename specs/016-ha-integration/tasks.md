...
- [ ] T014 Run the migration locally against a dev copy of `advisor_pgdata` (`alembic upgrade head`); verify the existing devices table accepts NULL mac_address via a manual SQL insert of a Thread-shaped row, then rolls back. If `alembic downgrade -1` fails, fix the migration before proceeding.
